...
using namespace tir;
static constexpr const char* kOperatorKind = "operator_kind";
...
/*!
 * \brief Replace each call_tir to PrimFunc which matches the kOperatorKind attribute with the
...
class AlterOpImplMutator : public ExprMutator {
 public:
  AlterOpImplMutator(const IRModule& mod, const Map<String, tir::PrimFunc>& op_primfunc_map,
                     const Map<String, Array<IndexMap>>& op_layout_map)
      : ExprMutator(mod),
        mod_(mod),
        op_primfunc_map_(op_primfunc_map),
        op_layout_map_(op_layout_map) {}
 ...
 private:
  Expr VisitExpr_(const CallNode* op) final {
    ...
    Optional<String> maybe_op_kind = old_func->attrs.GetAttr<String>(kOperatorKind);

    // If the callee does not have kOperatorKind attribute or no replacement is requested for
    // it, nothing to do here.
    if (!maybe_op_kind.defined() || op_primfunc_map_.count(maybe_op_kind.value()) == 0) return call;
    auto op_kind = maybe_op_kind.value();

    const auto& replacement_func = op_primfunc_map_[op_kind];
    const auto& buffer_transforms = op_layout_map_[op_kind];
  ...
  GlobalVar GetOrCreateGlobalVarForFunc(const PrimFunc& replacement_func, const String& op_kind) {
    ...
    // Retain the operator kind attribute on the replacement PrimFunc. This can help any future
    // passes that use kOperatorKind attribute to identify operator represented by a PrimFunc.
    attrs.Set(kOperatorKind, op_kind);
 ...
 private:
  ...
  const IRModule& mod_;
  /*! \brief Map from kOperatorKind attribute to the replacement PrimFunc */
  const Map<String, PrimFunc>& op_primfunc_map_;
  /*! \brief Map from kOperatorKind attribute to the layout transforms on i/o buffers */
  const Map<String, Array<IndexMap>>& op_layout_map_;
...
Pass AlterOpImpl(const Map<String, tir::PrimFunc>& op_primfunc_map,
                 const Map<String, Array<IndexMap>>& op_layout_map) {
  runtime::TypedPackedFunc<IRModule(IRModule, PassContext)> pass_func = [=](IRModule mod,
                                                                            PassContext pc) {
    return AlterOpImplMutator(mod, op_primfunc_map, op_layout_map).Run();
  };
